...
jQuery.extend({'zoombox':(function(){
	...
	return {'show':function(type,obj,url){
			switch(type)
			{
				...
				msg = 'æ¨ç¡®å®è¦å é¤åï¼';
				cnt = "<div class=\"modal-header\" style=\"height:2em;overflow:hidden;\"><button type=\"button\" class=\"close\" data-dismiss=\"modal\" aria-hidden=\"true\">Ã</button><h5>æä½ç¡®è®¤</h5></div><div class=\"modal-body\"><p class=\"alert alert-error\">"+msg+"</p></div><div class=\"modal-footer\"><button class=\"btn btn-primary\" onclick=\"javascript:submitAjax({'url':'"+url+"'});\">ç¡®å®</button><button class=\"btn\" onclick=\"javascript:$.zoombox.hide();\">åæ¶</button></div";
				break;

				case 'ajax':
				cnt = "<div class=\"modal-header\" style=\"height:2em;overflow:hidden;\"><button type=\"button\" class=\"close\" data-dismiss=\"modal\" aria-hidden=\"true\">Ã</button><h5>åºç°éè¯¯</h5></div><div class=\"modal-body\"><p class=\"alert alert-error\">"+obj.message+"</p></div>";
				break;

				case 'ajaxOK':
				cnt = "<div class=\"modal-header\" style=\"height:2em;overflow:hidden;\"><button type=\"button\" class=\"close\" data-dismiss=\"modal\" aria-hidden=\"true\">Ã</button><h5>æä½æå</h5></div><div class=\"modal-body\"><p class=\"alert alert-success\">"+obj.message+"</p></div>";
				break;

				default:
				cnt = "<div class=\"modal-header\" style=\"height:2em;overflow:hidden;\"><button type=\"button\" class=\"close\" data-dismiss=\"modal\" aria-hidden=\"true\">Ã</button><h5>åºç°éè¯¯</h5></div><div class=\"modal-body\"><p class=\"alert alert-error\">"+$(obj).attr('message')+"</p></div>";
			}
			m.html(cnt);
